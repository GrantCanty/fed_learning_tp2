...
class ScaffoldStrategy(Strategy):
    ...
    def configure_fit(
        ...
        if self.c is None:
            raise ValueError("Global control variate 'c' is not initialized. Ensure it's set during the first aggregation.")
    ...
    def aggregate_fit(
        ...
        for _, fit_res in results:
            ...
            cks.append([np.array(layer) for layer in ck])

        self.c = update_global_control_variate(self.c, cks)
...
def update_global_control_variate(c_global: List[np.ndarray], c_clients: List[List[np.ndarray]]) -> List[np.ndarray]:
    """Update the global control variate `c` based on client-side control variates `ck`."""
    num_clients = len(c_clients)
    updated_c = []

    for i in range(len(c_global)):  # for each layer's tensor
        sum_diff = sum((ck[i] - c_global[i]) for ck in c_clients)
        new_c_i = c_global[i] + (1.0 / num_clients) * sum_diff
