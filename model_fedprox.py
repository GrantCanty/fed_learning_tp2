...
class CustomFashionModel(nn.Module):
    ...
    def train_epoch(self, train_loader: DataLoader, criterion: nn.Module, optimizer: torch.optim.Optimizer, device: torch.device, global_model: nn.Module, proximal_term: float) -> tuple[float, float]:
        self.train()
        running_loss = 0.0
        correct = 0
        total = 0
        ...
        for inputs, labels in train_loader:
            ...
            running_loss += fedprox_loss.item() * inputs.size(0)
            _, predicted = outputs.max(1)
            correct += (predicted == labels).sum().item()
            total += labels.size(0)

        # averaging the fed prox loss so clients with high amts of samples don't have sway the model too much one way or another
        # more samples = higher loss value
        avg_fedprox_loss = running_loss / total
        accuracy = correct / total
        return avg_fedprox_loss, accuracy
